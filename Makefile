...
build-server-go-windows:
	...
	cd server/go && GOOS=windows GOARCH=amd64 go build -o bin/windows/rdpcloud_server.exe
...
build-client-go-linux:
	...
	cd client/go && GOOS=linux GOARCH=amd64 go build -o bin/linux/rdpcloud_client
